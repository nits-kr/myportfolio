...
import { useLoginMutation, useRegisterMutation } from '@/store/services/portfolioApi';
...
export default function LoginPage() {
    ...
    const onSubmit = async (data) => {
        try {
            let result;
            if (isLogin) {
                result = await login({ email: data.email, password: data.password }).unwrap();
            } else {
            ...
            // The authSlice extraReducers automatically handle state update on matchFulfilled
    ...
    const errorMessage = error?.data?.message || error?.error || "Authentication failed";
                ...
                <div className="glass-card p-5">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        >
                            ...
                            {isLoading ? 'Processing...' : (isLogin ? 'Access Dashboard' : 'Sign Up')}
                        </button>
                    </form>
